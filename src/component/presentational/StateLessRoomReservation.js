...
import {
    dumpValue, replaceItinerary
} from "./../../service/addCart/action";


import StateLessRoomResInfo from "./StateLessRoomResInfo";
import {array} from "prop-types";

class StateLessRoomReservation extends Component {
    static defaultProps = {
        isReview: false,
    ...
    componentDidMount() {
        ...
        // this.props.dumpValue()

        try {
            const {
                roomPriceInfo,
            } = this.props;
            let totalCost = (_sumBy(_map(roomPriceInfo, (each, i) => each.totalAmount))).toFixed(2)
    ...
    render() {
        ...
        let selectedCurrency = currencies[this.props.bookingDate.currency].symbol


        const {
            isPolicy,
            isMain, isAmenities, isContact, isDesc, isNear, startDate, endDate
        } = this.state
              ...
              <button className="selectNewTab" onClick={() => window.open(window.location, '_blank')} ><img src={openNewTab} /></button>
                        ...
                        {uniqueRoomListFin ?
                            _map(uniqueRoomListFin, (each, i) => {
                                return (
                                    <StateLessRoomResInfo
                                        {...this.props}
                                        roomCount={b[i].count}
                                        selectedRoomCount={selectedRoomCount}
                                        rateBy={rateBy}
                                        selectedCurrency={each.currency}
                ...
                <div className="d-flex flex-wrap justify-content-between webResp">
                    <div className={`flex-column tabInfoRoom ${isMain ? 'activeTab' : ''}`} style={{cursor: 'pointer'}}
                    ...
                    <div className={`flex-column tabInfoRoom ${isDesc ? 'activeTab' : ''}`} style={{cursor: 'pointer'}}
                         onClick={() => this.setState({
                             isDesc: true,
                             isAmenities: false,
                             isContact: false,
                    ...
                    <div className={`flex-column tabInfoRoom ${isAmenities ? 'activeTab' : ''}`}
                    ...
                    <div className={`flex-column tabInfoRoom ${isNear ? 'activeTab' : ''}`} style={{cursor: 'pointer'}}
                         onClick={() => this.setState({
                             isDesc: false,
                             isAmenities: false,
                             isContact: false,
                    ...
                    <div className={`flex-column tabInfoRoom ${isPolicy ? 'activeTab' : ''}`}
                         style={{cursor: 'pointer'}} onClick={() => this.setState({
                        isDesc: false,
                        isAmenities: false,
                        isContact: false,
                    ...
                    <div className={`flex-column tabInfoRoom ${isContact ? 'activeTab' : ''}`}
                         style={{cursor: 'pointer'}} onClick={() => this.setState({
                        isDesc: false,
                        isAmenities: false,
                        isContact: true,
                ...
                {
                    roomPriceInfo.length ?
                        <HotelInfo hotel={roomPriceInfo[0].hotel} isPolicy={isPolicy} rmvHtmlFunc={rmvHtmlFunc}
                                   isMain={isMain} isAmenities={isAmenities} isContact={isContact} isDesc={isDesc}
                                   isNear={isNear}/> : null
                }
                ...
                <HotelBookingProtection type='hotel' tripAmount={roomPriceInfo[0].fareBreakup.totalFare} /> 
                            ...
                            <div className="d-flex flex-wrap otherSectionBg">
                                <span className="overAmount"> Overall Amount : {selectedCurrency} {totolAmt}</span>
...
const mapDispatchToProps = dispatch => ({
    ...
    dumpValue: () => dispatch(dumpValue([])),
    replaceItinerary: (data) => dispatch(replaceItinerary(data))
})
export default connect(mapStateToProps, mapDispatchToProps)(withRouter(StateLessRoomReservation));
