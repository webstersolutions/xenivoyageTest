...
activity = () =>{
  ...
  render() {
    const { recentList, loginDetails, myTripList  ,activityTripList} = this.props;
    const userSession = JSON.parse(sessionStorage.getItem('loginInfo'));
        ...
        <ul>
          {recentList && recentList.length ?
            _map(recentList.reverse(), (each, i) => {
              ...
              return each.booking_type == "Taxi" && (
                <li key={i}>
                  <span>
                    <img src={img_transfer} alt='img_transfer' />
                  </span>
                  <span>
                    You booked a <b> {each.booking_type} </b> from  <b>  {" "}
                      {each.from}
                    </b>
                  </span> 
                  {/* <span>{each.hours}</span> */}
          ...
          {recentList && recentList.length ?
            _map(recentList.reverse(), (each, i) => {
              ...
              return each.booking_type == "Car" && (
                <li key={i}>
                  <span>
                    <img src={img_car} alt='img_car' />
                  </span>
                  <span>
                    You booked a <b> {each.booking_type} </b> from  <b>  {" "}
                      {each.from[0]}
          ...
          {recentList && recentList.length !=0 ?
            _map(recentList.reverse(), (each, i) => { 
              return each.booking_type == "Activity" && (
                <li key={i}>
                  <span>
                    <img src={img_activities} alt='img_activities' />
                  </span>
                  <span>
                    You booked a <b> {each.booking_type} </b> to 
                      <b> {each.item && each.item.response.data && each.item.response.data.itemSummaries[0].productTitle}</b> at <b>  {" "}
                      {each.item && each.item.response.data && each.item.response.data.itemSummaries[0].departsFrom}
                    </b>
                  </span> 
                </li>
              )
