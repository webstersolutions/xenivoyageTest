...
class Itinerary extends React.Component {
  ...
  diffPassangersCheck = dragItem => { 
   if( this.props.itineraryList.type != "package"){
    if (
      this.props.itineraryList.find(wishListData => {
        if (wishListData.type !== "package") {
  ...
  onEmailSend = (nextProps) => {
    ...
    const packages = _filter(payload, function(each) {
      return each.type == "package";
    });
      ...
      <div>
        ...
        <ul className="rateShowTable">
          {item.bookingData.room.map((value, index) => {
            ...
            let totalBaseFare = roomCnt * (Number(value.basedAmount) + Number(value.farebreakupAmount))
            let bseFare = +totalBaseFare / +item.bookingData.bookingDays;
            ...
            return (
              ...
              <li>
                <span>
                  {value.name} <strong>X</strong> {value.roomCount}
                </span>{" "}
                <span>
              ...
              <span style={{fontWeight: "normal"}}>
                {currencies[item.bookingData.currency].symbol} {item.taxFees}
              </span>
            </li>
            </span>
            );
  ...
  renderPackage = item => {
    ...
    const date =new Date(item.bookingData.location.fromDate)
    
    let totalTravellers = 0;
    let traveller = _map(item.totalTravellers,
      ({count, pricePerTraveler, type}) => {
      ...
      // }
      totalTravellers += count;
      return ` ${count} ${type}${separation} `;
    });
    return (
      <div>
        <h2>
          <img src={img_packages} />{" "}
          <span>{item.selectedPackageInfo.country}</span>
        </h2>
        <ul>
          <li>
            <div className="timeShow">
              ...
              <span style={{ fontSize: "11px" }}>
                {/* {item.selectedActivityInfo.duration} */}
            {    (item.selectedPackageInfo.duration.type == "multiday") ? 
                    ( " "+ item.selectedPackageInfo.duration.time-1 + " Nights" + " | " + item.selectedPackageInfo.duration.time + " Days" ):
                     ( packageDetails.duration.time + " Hours")
    }
        ...
        <ul className="rateShowTable">
          ...
          <li>
            ...
            <span>{item.bookingData.totalTravellers[0].count} Adult ,{item.bookingData.totalTravellers[1].count} Child , {item.bookingData.totalTravellers[2].count} Sr. Citizen</span> 
              ...
              <ul className="itinerarySelItems">
                {_map(itineraryList, (item, index) => {
                  ...
                  if (item.type == "package") {
                    title = item.selectedPackage.title;
                  }
                  if (item.type == "car") {
                    displayImg = img_car;
                    ...
                    displayImg = img_packages;
                    displayView = this.renderPackage(item);
                  }
                  ...
                  return (
                    <li key={index}>
                      <div className="itineraryTitle">
                        <div>
                          ...
                          <h4
                            ...
                            (item.type == "package" ? title : item.title)   : title}
                          >
                          ...
                          {item.type == "package" ?
                          (
                            <span style={__titleStyle} title={title}>
                            {title}
                          </span>
                      ...
                      {item.subtitle} <img src={img_DateArrow} alt="" /> IAH
                          ...
                          <h4
                              title={item.type == "package" ? title : item.title}
                              >
                                {/* {item.type.toUpperCase()} - */}
                                {item.type == "package" ? title : item.title}
                                <span style={__titleStyle} title={title}>
                                  {title}
                                </span>
                                {/* <span style={__titleStyle} title= {item.subtitle}>
