...
import {packageBook, packageGetPayload} from "../../../service/package/action";
...
class PackagePayment extends Component {
    ...
    componentDidMount() {
       ...
       /* if (this.props.selectedPackage) {
            const questions = _clone(this.props.selectedPackage.bookingQuestions);

            this.setState({
                travellerQuestions: _map(this.props.selectedPackage.passengerAttributes, attr => {
                    const matchedQuestion = _find(this.props.selectedPackage.bookingQuestions, q => q.stringQuestionId === attr.questionId);
    ...
    componentWillReceiveProps(nextProps) {
        ...
        if (nextProps.selectedPackage !== this.props.selectedPackage && nextProps.selectedPackag) {
        ...
        if (nextProps.totalTravellers > 0 && nextProps.totalTravellers !== this.props.totalTravellers) {
            ...
            if (nextProps.selectedPackage.allTravellerNamesRequired && nextProps.totalTravellers) {
    ...
    handleBooking = values => {
       ...
       // console.log(value);
        ...
        const {selectedPackage: {bookingQuestions}, totalTravellers} = this.props;
      ...
      //  console.log(values);
        ...
        _times(totalTravellers, i => {
            travellers.push({
                bandId: this.props.selectedPackagePrice.ageBands[0].bandId,
        ...
        if (value.hotelId !== undefined) {
            _forEach(this.props.packageHotelList, hotel => {
        ...
        if (
            ...
            const guestFlag = "PackageBookingFlag";
            ...
            const payloadInfo = {
                paymentMethod: {
                    ...
                    paymentBreakup: [
                        {
                            amount: this.props.selectedPackagePrice.retailPrice,
                            currency: this.props.selectedPackagePrice.currencyCode,
                            type: "Cash"
                        }
                    ]
                },
                bookingPayload: [
                    {
                        demo: true,
                        currencyCode: this.props.selectedPackagePrice.currencyCode,
                        booker: {
                            email: value.email,
                            homePhone: "",
                        ...
                        items: [
                            {
                                ...selectedHotel,
                                travelDate: this.props.selectedPackagePrice.bookingDate,
                                productCode: this.props.selectedPackage.code,
                                tourGradeCode: this.props.selectedPackagePrice.gradeCode,
                        ...
                        totalAmount: this.props.selectedPackagePrice.retailPrice,
                        merchantAmount: this.props.selectedPackagePrice.merchantNetPrice
                    }
                ],
                email: value.email,
            ...
            this.props.packageGetPayload(payloadInfo);
            ...
            const payload = {
                paymentMethod: {
                    ...
                    paymentBreakup: [
                        {
                            amount: this.props.selectedPackagePrice,
                            currency: "USD",
                            type: "Cash"
                        }
                    ]
                ...
                bookingPayload: [
                    {
                        ...
                        items: [
                            {
                                ...
                                travelDatefrom: this.props.package_date,
                                travelDateto: this.props.endson,
                                productCode: this.props.selectedPackage.packageId,
                               // tourGradeCode: this.props.selectedPackagePrice.gradeCode,
                               // languageOptionCode: value.languageOptionCode,
                               package_cancellation:this.props.selectedPackage.package_cancellation,
                                bookingQuestionAnswers,
                                specialRequirements: value.specialRequest,
                                travellers:{ 
                                    firstname: value.firstName0,
                        ...
                        participants:{
                            adult:parseInt(this.props.num_adults),
                            children:parseInt(this.props.num_child),
                            seniorCitizen:parseInt(this.props.num_sr_citizen),
                        
                        },
                        billingDetails:{
                            firstName: value.firstName0,
                            lastName:  value.lastName0,
                            email: value.email,
                        ...
                        packageId : parseInt(this.props.selectedPackage.packageId),
                    eventId : parseInt(this.props.eventId),
                    transDetails : {
                    transId : "trans123",
                    transDate : "2020-07-01 14:00:00",
                    transAmount : parseFloat(this.props.selectedPackagePrice),
                    paymentMethod: "cash"
                },
                        images: this.props.selectedPackage.gallery,
                        featuredImage:this.props.selectedPackage.featuredImage,
                        totalAmount: parseFloat(this.props.selectedPackagePrice),
                        merchantAmount: "0",
                    }
                ],
                email: value.email,
            ...
            this.props.packageBook(payload);
        }
    };
    handleCheckClick = () => {
    ...
    render() {
        const {
            ...
            selectedPackage, selectedPackagePrice, packageHotelList, totalTravellers
        } = this.props;
        let response;
        // const {retailPrice} = selectedPackagePrice
        const retailPrice = selectedPackagePrice && selectedPackagePrice.retailPrice
                ...
                >
                    <div>
                        ...
                        <div className="paymentDetails">
                            ...
                            {
                                selectedPackage && (selectedPackage.allTravellerNamesRequired || !_isEmpty(_get(selectedPackage, 'passengerAttributes', [])))
                                    ? _times(totalTravellers, i => (
                                        <React.Fragment>
                            ...
                            {
                                selectedPackage && selectedPackage.hotelPickup == true && <div className="row">
                                    {
                                        <div className="col-xl-9 col-lg-9 col-md-9">
                                            <div className="form-group">
                                                {packageHotelList.length > 0 ? (<Field
                                                    name="hotelId"
                                                    type="text"
                            ...
                            {selectedPackagePrice && selectedPackagePrice.langServices && <div className="row">
                                <div className="col-xl-5 col-lg-5 col-md-5">
                                    <div className="form-group">
                                        <Field
                                            name="languageOptionCode"
                                            type="text"
                                            label="Tour/Package Language *"
                                            component={SelectField}
                                            placeholder="Tour/Package Language"
                                            validate={this.dynamicValidation.selectOption}
                                        >
                                            <option value="null">Choose Language</option>
                                            {
                                                _map(selectedPackagePrice.langServices, (value, key) => (
...
const mapDispatchToProps = dispatch => ({
    ...
    packageGetPayload: payload => dispatch(packageGetPayload(payload)),
    ...
    packageBook: payload => dispatch(packageBook(payload)),
});
...
PackagePayment.propTypes = {
    isdivHide: propTypes.bool
};
PackagePayment.defaultProps = {
    loginDetails: {}
};
